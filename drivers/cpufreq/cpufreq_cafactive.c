...
struct cpufreq_cafactive_policyinfo {
...
static DEFINE_PER_CPU(struct cpufreq_cafactive_policyinfo *, polinfo);
...
static struct cpufreq_cafactive_tunables *cached_common_tunables;
...
static void cpufreq_cafactive_timer_resched(unsigned long cpu,
					      bool slack_only)
{
	struct cpufreq_cafactive_policyinfo *ppol = per_cpu(polinfo, cpu);
 ...
 */
static unsigned int choose_freq(struct cpufreq_cafactive_policyinfo *pcpu,
		unsigned int loadadjfreq)
{
...
static void cpufreq_cafactive_timer(unsigned long data)
{
	...
	u64 cputime_speedadj;
	int cpu_load;
	struct cpufreq_cafactive_policyinfo *ppol = per_cpu(polinfo, data);
	struct cpufreq_cafactive_tunables *tunables =
		ppol->policy->governor_data;
	struct cpufreq_cafactive_cpuinfo *pcpu;
	...
	for_each_cpu(i, ppol->policy->cpus) {
		...
		trace_cpufreq_cafactive_cpuload(i, tmploadadjfreq /
						  ppol->policy->cur);
	...
	if (new_freq < ppol->target_freq &&
		...
		trace_cpufreq_cafactive_notyet(max_cpu, cpu_load,
	...
	trace_cpufreq_cafactive_target(max_cpu, cpu_load, ppol->target_freq,
					 ppol->policy->cur, new_freq);
...
static int cpufreq_cafactive_speedchange_task(void *data)
{
	...
	cpumask_t tmp_mask;
	unsigned long flags;
	struct cpufreq_cafactive_policyinfo *ppol;
	...
	while (1) {
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			ipa_cpufreq_requested(ppol->policy, ppol->target_freq);
#endif
...
static struct cpufreq_cafactive_policyinfo *get_policyinfo(
					struct cpufreq_policy *policy)
{
	struct cpufreq_cafactive_policyinfo *ppol =
	...
	ppol->policy_timer.function = cpufreq_cafactive_timer;
	init_timer(&ppol->policy_slack_timer);
	ppol->policy_slack_timer.function = cpufreq_cafactive_nop_timer;
...
static struct cpufreq_cafactive_tunables *get_tunables(
				struct cpufreq_cafactive_policyinfo *ppol)
{
	if (have_governor_per_policy())
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		ppol->last_evaluated_jiffy = get_jiffies_64();
		cpufreq_cafactive_timer_start(tunables, policy->cpu);
	...
	case CPUFREQ_GOV_LIMITS:
		...
		if (ppol->governor_enabled) {
			...
			if (policy->min < ppol->min_freq)
				cpufreq_cafactive_timer_resched(policy->cpu,
								  true);
			ppol->min_freq = policy->min;
		}
