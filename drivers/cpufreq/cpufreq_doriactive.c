/*
 *  drivers/cpufreq/cpufreq_doriactive.c
 *
...
#define MICRO_FREQUENCY_DOWN_THRESHOLD		(25)
...
/* cpufreq_doriactive Governor Tunables */
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_DORIACTIVE
static
#endif
struct cpufreq_governor cpufreq_gov_doriactive = {
	.name			= "doriactive",
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	dbs_wq = alloc_workqueue("doriactive_dbs_wq", WQ_HIGHPRI, 0);
	if (!dbs_wq) {
		printk(KERN_ERR "Failed to create doriactive_dbs_wq workqueue\n");
		return -EFAULT;
	}

	ret = cpufreq_register_governor(&cpufreq_gov_doriactive);
	if (ret)
...
{
	cpufreq_unregister_governor(&cpufreq_gov_doriactive);
	destroy_workqueue(dbs_wq);
	kfree(&dbs_tuners_ins);
}

MODULE_AUTHOR("Alexander Clouter <alex@digriz.org.uk>");
MODULE_DESCRIPTION("'cpufreq_doriactive' - A dynamic cpufreq governor for "
		"Low Latency Frequency Transition capable processors "
		"optimised for use in a battery environment");
MODULE_LICENSE("GPL");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_DORIACTIVE
